...
def cluster_interfaces(entry, afs_thr, p_thr, d_thr):
    ...
    for i in range(5):
        ...
        # store results
        ids_p = np.where(m)[0]
        ids_interfaces.append([[int(v) for v in ids_p[ids]] for ids in ids_ints])
...
def main():
    ...
    # write output file
    json.dump(interfaces, open("datasets/clustered_interfaces.json", 'w'))
