...
def reg_scoring(y, p):
    ...
    return {
        'mse': pt.mean(pt.square(y - p)).cpu().numpy(),
        'mae': pt.mean(pt.abs(y - p)).cpu().numpy(),
        'rmse': pt.sqrt(pt.mean(pt.square(y - p))).cpu().numpy(),
        'pcc': pearsonr(y.cpu().numpy(), p.cpu().numpy())[0] if not pt.allclose(y,y[0]) else np.nan,
        'auc': roc_auc_score((y>0.0).float().cpu().numpy(), pt.sigmoid(p).cpu().numpy()) if not (pt.all(y>0.0) or pt.all(y<=0.0)) else np.nan,
        'std': pt.std(p).cpu().numpy(),
    }
