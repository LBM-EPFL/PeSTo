...
def select_complete_assemblies(dataset, m):
    # assemblies key
    akeys = np.array(['/'.join(key.split('/')[:3]) for key in dataset.keys])

    # get non-selected assemblies
    rmkeys = np.unique(akeys[~m])

    # select all assemblies not in the non-selected assemblies
    return ~np.isin(akeys, rmkeys)
